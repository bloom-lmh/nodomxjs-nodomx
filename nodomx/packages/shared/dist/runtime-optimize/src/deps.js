export function normalizeDependencyPath(path) {
    if (!path) {
        return;
    }
    let value = path.trim();
...
export function mergeDependencyPaths(target = [], next) {
    if (!next) {
        return target;
    }
...
export function isRelatedDependencyPath(left, right) {
    if (!left || !right) {
        return false;
    }
    return left === right
...
export function hasDependencyMatch(paths, dirtyPaths) {
